...
function pageInit(sectionId,definedFn) {
    if(!$("#"+sectionId).hasClass("page_active")){
        contentHeight();
        $("section.ui-page .comp").each(function () {
        ...
        if(data_len != 0){
              $.map(dataObj.data,function(data_list){
                ...
                if(data_list.telephone){
                    group_centent_obj.find(".nav_list_box_right a").first().attr("href","sms:"+data_list.telephone);
                    group_centent_obj.find(".nav_list_box_right a").last().attr("href","tel:"+data_list.telephone);
                  }
            ...
            domObj.find(".list_data .nav_list_box_left").die().live("tap", function () {
                ...
                if(compObj.selectOne){
                    ...
                    if (typeof compObj.selectBack == "function") {
                        ...
                        compObj.selectBack($(this).parent(),obj_id,compObj,"selectOne");
                    }
                }else{
 ...
 */
function userSelect(obj,objId,compObj,selectRadio){
 ...
 * @param selectRaido  选人——单选
 */
function createDialog(popupUrl,parentId,popupId,dialogExpandFn,selectRaido){
    $.ajax({
        url: popupUrl,
        async: false,
        success: function(html){
            if(selectRaido == "radio"){
                var pupup_obj = $(html);
